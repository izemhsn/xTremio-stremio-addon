...
| `CACHE_MAX_SORTED_CATALOGS` | `64` | How many sorted catalog views to hold across all accounts. A shelf is sorted once and then paginated from that view, instead of copying and re-sorting the whole list for every page — on a 50k-title account that is 14-16 ms of blocking work per page, on the single thread that also relays video. An entry holds one machine word per item, but it keeps the list it was sorted from reachable, which is why the count is bounded. One account can browse many genres, so this is sized for a user moving around rather than for one shelf. |
